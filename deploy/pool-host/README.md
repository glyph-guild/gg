...
**The registry listens on loopback too, at `127.0.0.1:5000`.** Every strategy
pins its member image there (`127.0.0.1:5000/...@sha256:...`), and the proxy
lets a build tag and a push reach that address and no other. `compose.yaml`
starts it beside the proxy — `registry:2`, pinned by digest, with a named volume
so images survive a restart. It is not given the socket, and it is on loopback
for the proxy's reason: a registry that answered on the network would take
pushes from anybody who reached the port, and a push decides what a member runs.
Docker trusts a loopback registry without TLS, so nothing else is configured.
...
**With cloud-init**, which is steps 1–3 already done, and step 4 begun: hand
...
**pinned** pool-host bundle, starts the proxy and the registry, writes the `gg`
user's `config.json`, and links the unit — then stops,
...
3. Put `gg` on the machine. **Two shapes, and they update differently:**
   ...
   **All three files, beside each other** — the archive is the whole publish
   directory, and the install is too. .NET loads a native library on first
   call, not at start, so a `gg` installed alone prints its version cleanly and
   dies the first time it hosts a terminal (`libporta_pty`); `libonigwrap`
   arrives through Terminal.Gui, and a list that leaves one out is right only
   until something calls it. `install` replaces each file rather than writing
   into it, so a running `gg` is never overwritten in place.

   This shape has no update path at all, by design — see below.
4. **Name the control plane, in both places it is read, before anybody signs
   in.** Set `GG_POOL` and `GG_CONTROL_PLANE` in the unit file (or the drop-in
   cloud-init writes), then `systemctl daemon-reload`; and give the `gg` user
   the same address:

   ```sh
   sudo -u gg -i gg config set control-plane https://<the same address>
   ```

   The unit's environment reaches only the process systemd starts. `gg login`,
   typed by a person, reads the `gg` user's own `config.json` — and without it
   signs in to `http://localhost:5199`, which on this machine is nothing.
5. **A person signs in on the machine,** as the `gg` user: `gg login`. This is
   a device flow — the person approves in their own browser, and the session
   lands here. Then `systemctl enable --now gg-runner-maintain`.
...
itself: nothing is unset, nothing is refused, and it looks configured. The unit
file, cloud-init's drop-in and the `gg` user's `config.json` all name it for
that reason — the first two for the unit, the third for the person signing in.

## A person is needed once, and again only after an outage

**Step 5 is a person, and after it the host restarts on its own.** The runner
...
**The runner renews its own credential**, within three days of expiry, with
nobody at the machine — `gg runner maintain` asks as it runs. A revoked
credential is not renewed, and neither is one that already ran out: a host that
was down when its thirty days ended needs a person to sign in again, and the
refusal names that so it does not read like a broken machine.
