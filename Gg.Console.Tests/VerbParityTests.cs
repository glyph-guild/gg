...
public class VerbParityTests
{
    ...
    internal static readonly IReadOnlyDictionary<string, string> Decided =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["RunnerWatch"] = "RUN, from the runner modal, on `w`. This entry used to say "
                            + "SUGGESTED - the pane named the command and a person ran it "
                            + "themselves - on the grounds that a WebRTC handshake inside "
                            + "the render loop would put a twenty second wait between a "
                            + "keystroke and a frame. That reasoning was right and the "
                            + "conclusion did not follow: the watch is STARTED between "
                            + "sessions and never waited for, so the console is down for "
                            + "the instant a session takes to rebuild rather than for "
                            + "however long a runner takes to answer. Every step of the "
            ...
            ["CredentialSend"] = "RUN, from the runner modal, on `c`, while the machine is "
                               + "beating. Beside `w` and on its terms, because it is the "
                               + "same mechanism - an introduction over the control plane, "
                               + "picked up on a heartbeat - which is why it is offered over "
                               + "somebody else's runner as readily as over this one. "
                               + "BETWEEN SESSIONS, and it needs that more than watching "
                               + "does: it reads a secret with the echo off, which a "
                               + "Terminal.Gui session cannot arrange at all. It asks for "
