...
public sealed record AllowanceWindow
{
    ...
    /// <remarks>
    /// Input, output, cache read and cache write added together. How a provider
    /// weighs one against another is not published, so a weighting here would
    /// be a guess wearing a decimal point — and the ceiling is configured
    /// against this same arithmetic, which is what makes a fraction mean
    /// anything.
    /// </remarks>
...
public sealed record AllowanceReading
{
    ...
    public static string? Validate(AllowanceReading reading)
    {
        ...
        foreach (var window in reading.Windows)
        {
            ...
            if (window.Tokens < 0)
            {
                return $"'{window.Kind}' reports {window.Tokens} tokens, and a window "
                     + "cannot hold fewer than none.";
            }
