...
public class FocusLandsWhenTheTabChangesTests
{
    ...
    {
        // A MODAL THAT IS STILL ONE PLACE, which is what this rule is about.
        // It used to be Help, and Help stopped being an example when its pages
        // became documents with their own cursors - a modal made of parts has
        // to say which part, and where each of those lands is asserted beside
        // the part. The rule here is the one every SIMPLE modal shares: it
        // takes the focus once and then it is left alone.
        await Assert.That(FocusChange.Wanted(UiMode.ConfirmGround, TabId.Runners, TabId.Runners, false))
            .IsEqualTo(FocusTarget.Modal)
            .Because("a modal owns the keyboard, so it has to hold the focus too.");

        await Assert.That(FocusChange.Wanted(UiMode.ConfirmGround, TabId.Runners, TabId.Runners, true))
            .IsEqualTo(FocusTarget.LeaveAlone)
