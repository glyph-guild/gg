...
public class AFlightIsAskedWhatItIsForTests
{
    ...
    {
        var asked = Press(Browsing("hal-score", "research"), KeyStroke.Char('f'));
    ...
    {
        var flown = Press(Browsing(), KeyStroke.Char('f'));
    ...
    {
        var asked = Press(Browsing("hal-score"), KeyStroke.Char('f'));
        var escaped = Press(asked, KeyStroke.Esc);

        await Assert.That(escaped.Mode).IsEqualTo(UiMode.Normal);

        await Assert.That(escaped.AskingKindFor).IsEqualTo(ComposingFor.NewFlight)
