...
public class AFlightCanOpenItsLinkTests
{
    ...
    /// <summary>
    /// The shape a sweep's flight arrives in: a url and nothing else.
    /// </summary>
    /// <remarks>
    /// <b>A neutral host, because this repository is public and names no
    /// forge.</b> The first draft of this file pasted the real url from the
    /// flight that reported the defect, which <c>ProviderNeutralityTests</c>
    /// caught - the boundary that guard keeps is the same one this whole
    /// binary keeps, and a fixture is no exception to it.
    /// </remarks>
    private static FlightIntent ALink() => new()
    {
        Kind = FlightIntentKinds.Uri,
        Uri = "https://tracker.example/acme/_workitems/edit/18490",
    };
    ...
    public async Task The_link_it_opens_is_the_flights_own()
    {
        await Assert.That(FlightDetails.LinkHere(Reading(ALink())))
            .IsEqualTo("https://tracker.example/acme/_workitems/edit/18490");
